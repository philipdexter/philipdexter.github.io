...
From this we get a speedup of $3.6\mathrm{x}$ and an accuracy loss of $11\%$.
...
which has an impressive speedup of $7.9\mathrm{x}$ at an accuracy loss
of $16\%$.
...
perforation of $50\%$ to be the best result which gives a speedup of $1.3\mathrm{x}$
...
\textrm{perforation} & \rlap{\textrm{accuracy loss across 10 test runs}} \\\hline
15\% &
10.9\% &
13.8\% &
13.5\% &
...
a speedup of $2.8\mathrm{x}$ and an accuracy loss of $15.5\%$.
If the user wishes to move upwards into the $3\mathrm{x}$ speedup range
...
Below are the results over $10$ runs.

$$
\begin{array}{r|llllllllll}
& \rlap{\textrm{speedup and accuracy loss across 10 test runs}} \\\hline
\textrm{speedup} &
2.7\mathrm{x} &
2.7\mathrm{x} &
2.8\mathrm{x} &
